...
// ReqHeaders variable and return a challenge otherwise.

int CheckAuth(char *szPath, ReqHeaders *rhInfo)
{
  ...
  while (bNotFound)
    {
      ...
      strcat(szTmpPath, szAccessName);  // Create filename.
      iRc = stat(szTmpPath, &sBuf);
      if (iRc == 0) // We found the file.
        {
          iRc = CheckFile(szTmpPath, rhInfo);
          bNotFound = FALSE;
          continue;
        }
    }
...
int CheckFile(char *szFile, ReqHeaders *rhInfo)
{
  ifstream ifIn;
  ...
  if (stricmp(szType, "basic") == 0) // Check basic authentication.
    {
      if (rhInfo->szAuth == NULL) // Ask for credentials
        {
          rhInfo->szRealm = strdup(szRealm);  // Return this realm.
          iRc = ACCESS_DENIED;
        }
      else // Check their credentials
        {
          iRc = BasicCheck(szUserFile, rhInfo);
        }
    }
...
int BasicCheck(char *szFile, ReqHeaders *rhInfo)
{
  char *szUserPass,
       *szBuf,
       *szClientUser,
  ...
  szCode = new char[strlen(rhInfo->szAuth)];
  sscanf(rhInfo->szAuth, "%*s %s", szCode);
  ...
  while ((! ifPass.eof()) && (bFound == FALSE))
    {
      ...
      *szPass = NULL;
      ...
      if ( (stricmp(szClientUser, szUser) == 0) &&
        ...
        {
          rhInfo->szRemoteUser = strdup(szClientUser);
          rhInfo->szAuthType = strdup("basic");
          bFound = TRUE;
        }
      memset(szBuf, 0, SMALLBUF);
    }
