...
int main(int argc, char *argv[])
{
  ...
  cerr << "w3d> Starting server on port number " << iPort << "." << endl;
...
//  cerr << "W3Conn. Client connected." << endl;   
...
//  cerr << "Request Line: " << sClient->szOutBuf << endl;
...
void DoHttp09(Socket *sClient, char *szRequest, char *szUri)
{
  ...
  iRc = CheckAuth(szBuf, &rhInfo);  // Check for authorization.
  if (iRc != ACCESS_OK)
    {
      ...
      if ( *szFrom  == '%' )  // Look for the % sign.
        {
          ...
          if (*szFrom)        // If valid.
            {
              *szTo = Hex2Dec(*szFrom) * 16;       // Convert.
              szFrom++;
            }
          if (*szFrom) *szTo += Hex2Dec(*szFrom);  // Add remainder.
        }
      else
        {
