           ...
           char *szSearch, ReqHeaders *rhInfo, char *szUri)
{
  ...
  // Post not allowed on a path request.
  if (stricmp(szMethod, "post") == 0)
    {
      iRc = SendError(sClient, "POST method not allowed for this URL.", 
                      401, rhInfo);
      return iRc;
    }
      
  // Method must be GET or HEAD. Treat them the same until time to
  // send the data.
...
//              cerr << "wrote " << i+1 << " bytes" << endl;
            }
          cerr << "bytes read from socket: " << iCount << endl;
        }
      else  // Binary data.
        {
      ...
      cerr << "post file: " << szFile << endl;
      struct stat s;
      stat(szFile, &s);
      cerr << "stat size = " << s.st_size << endl;
    }
...
//  cerr << "parsing cgi output" << endl;
  ...
  sprintf(szBuf, "Content-Length: %d\r\n\r\n", sBuf.st_size - iCount);
  sClient->Send(szBuf);
  cerr << "raw file length = " << sBuf.st_size << endl;
  cerr << "Sending content-length = " << sBuf.st_size - iCount << endl;

//  cerr << "checking method = " << szMethod << endl;
  if (strcmp(szMethod, "HEAD") != 0) // Only send the entity if not HEAD.
    {
      rhInfo->ulContentLength = sBuf.st_size - iCount;
//      cerr << "sending output file now. bytes = " << rhInfo->ulContentLength << endl;
...
//  if (cParms->szPost != NULL) unlink(cParms->szPost);
  cerr << "output file from client: " << cParms->szOutput << endl;
  cerr << "post file to client: " << cParms->szPost << endl;
  delete cParms;
