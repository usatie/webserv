...
int ExecCgi(Cgi *cParms)
{
  ...
  iEnvLen = strlen(cParms->rhInfo->szQuery) + (13 * 64) + 32;
  szEnvStrings = new char[iEnvLen];
  memset(szTmp, '\0', iEnvLen);
  szTmp = szEnvStrings;

  sprintf(szRequestMethod, "REQUEST_METHOD=%s", cParms->rhInfo->szMethod);
  memcpy(szTmp, szRequestMethod, strlen(szRequestMethod));
  szTmp += strlen(szRequestMethod) + 1;

  sprintf(szScriptName, "SCRIPT_NAME=%s", cParms->rhInfo->szUri);
  memcpy(szTmp, szScriptName, strlen(szScriptName));
  szTmp += strlen(szScriptName) + 1;

  if (cParms->rhInfo->szQuery != NULL)
    {
      szQueryString = new char[(strlen(cParms->rhInfo->szQuery) + 15)];
      sprintf(szQueryString, "QUERY_STRING=%s", cParms->rhInfo->szQuery);
    }
  else
    {
  ...
  if (cParms->rhInfo->szAuthType != NULL)
    {
      sprintf(szAuthType, "AUTH_TYPE=%s", cParms->rhInfo->szAuthType);
      sprintf(szRemoteUser, "REMOTE_USER=%s", cParms->rhInfo->szRemoteUser);
    }
  else
    {
  ...
  if (cParms->rhInfo->szContentType != NULL)
    {
      sprintf(szContentType, "CONTENT_TYPE=%s", cParms->rhInfo->szContentType);
    }
  else
    {
  ...
  if (strcmp(cParms->rhInfo->szMethod, "POST") == 0)
    {
      sprintf(szContentLength, "CONTENT_LENGTH=%d", cParms->rhInfo->ulContentLength);
    }
  else
    {
  ...
  bRc = CreateProcess((cParms->szProg),   // Command line.
                      NULL,
...
void InitCgi()
{
  ...
  strcpy(szServerProtocol, "SERVER_PROTOCOL=HTTP/1.0");
